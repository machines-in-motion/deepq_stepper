...
terr_gen.create_box([0.59,  -0.05, 0], [0., 0, 0.1, 1], '3cm')
terr_gen.create_box([0.50,  0.05, 0], [0, 0.1, 0, 1], '4cm')

terr_gen.create_box([0.4,  0.3, -0.0], [0, 0, 0, 1], '4cm')
terr_gen.create_box([0.4,  -0.3, -0.0], [0, 0, 0, 1], '4cm')

terr_gen.create_box([0.7,  -0.2, 0.02], [0, 0, 0., 1], '3cm')
terr_gen.create_box([0.7,  0.2, 0.04], [0, 0, 0., 1], '3cm')

terr_gen.create_box([0.9,  0.2, 0.04], [0, 0, 0., 1], '3cm')
terr_gen.create_box([0.9,  0.0, 0.04], [0, 0, 0., 1], '4cm')
terr_gen.create_box([0.9,  -0.2, 0.04], [0, 0, 0., 1], '3cm')
...
p.resetDebugVisualizerCamera( cameraDistance=0.9, cameraYaw= 125, cameraPitch=-80, cameraTargetPosition=[0.40,0,0])

bolt_env.start_recording("3d_stepping_stones_top_view_no_lines.mp4")
